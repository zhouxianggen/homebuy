...
import com.hg.www.buyer.data.api.OrderApi;
...
public class OrderService extends BasicService {
    ...
    public String pull(String args) {
        String server = AppSettings.getOrdersServerAddress();
        ...
        try {
            ...
            for (int i = 0; i < sellers.length(); i++) {
                ...
                OrderApi.getInstance().sync(seller);
            }
    ...
    public void onFinished(String error) {
        BusApi.getInstance().postToMainThread(new ServicePullDataFinishedEvent(
                ServicePullDataFinishedEvent.SERVICE_ORDER, error
        ));
    }
}
